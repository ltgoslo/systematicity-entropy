...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="The input bucket size for each level of relative position.",
    )
    parser.add_argument(
        "--output_bucket_size",
        type=int,
        default=32,
        help="The output bucket size for each level of relative position.",
    )
    parser.add_argument(
        "--rope_theta",
        type=int,
        default=10_000,
        help="The angle change for rope.",
    )
    parser.add_argument(
        "--patience",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="van",
        choices=["van", "rope", "rel", "learn"],
        help="Which Transformer to use",
    )
    return parser.parse_args()


def get_loss_from_transformer(batch: tuple, model: nn.Module) -> torch.Tensor:
    source_ids, source_mask, target_ids, target_mask = batch
    logits = model(source_ids, source_mask, target_ids[:, :-1], target_mask[:, :-1])
...
def get_accuracy_from_transformer(
...
def train(
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            loss = get_loss_from_transformer(batch, model)
            loss.backward()
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader):
                ...
                val_loss += loss.item()
                if args.acc:
                    correct += get_accuracy_from_transformer(
...
def main():
    ...
    wandb_config = {
        ...
        "input_bucket_size": args.input_bucket_size,
        "output_bucket_size": args.output_bucket_size,
    }
    wandb.init(project=args.wandb, config=wandb_config)

    if args.type == "van":
        from systematicity_entropy.models.transformer import Transformer
    elif args.type == "rope":
        from systematicity_entropy.models.transformer_rope import Transformer
    elif args.type == "rel":
        from systematicity_entropy.models.transformer_relative import Transformer
    elif args.type == "learn":
        from systematicity_entropy.models.transformer_learned import Transformer
    ...
    model = Transformer(
        ...
        args.rope_theta,
        args.input_bucket_size,
        args.output_bucket_size,
    ).to(DEVICE)
